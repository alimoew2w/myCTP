...
dataFile = os.path.join(mainEngine.DATA_PATH, accountInfo['accountID'], 'TickData', 
                        (vtFunction.tradingDay()+ '.csv'))
...
import pandas as pd
try:
    dataFileOld = os.path.join(mainEngine.DATA_PATH,accountInfo['accountID'],'ContractInfo',
        (vtFunction.tradingDay() + '.csv'))
    dataFileNew = os.path.normpath(os.path.join(ROOT_PATH,'main','contract.csv'))
    if os.path.exists(dataFileOld):
        ## ---------------------------------------------------------------------
        # with open(dataFileOld,'ab') as f:
        #     #将 dataFileNew 内容追加到 dataFileOld 的后面
        #     f.write(open(dataFileNew,'rb').read()) 
        ## ---------------------------------------------------------------------
